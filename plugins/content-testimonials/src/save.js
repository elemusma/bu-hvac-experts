  ...
  attributes.testimonials.map((testimonial, index) => {
    return (
        <div className={`position-relative text-center ${testimonial.col_class}`} style={`${testimonial.col_style}`}>
		  ...
		  <p className={`bold`} style={{ cursor: 'pointer', marginBottom:'0px' }}><RichText.Content value={testimonial.title} /></p>
		  { testimonial.company && ( 
			  <p className={`small`} style={{ cursor: 'pointer',color:'#acacac', marginTop:'0px' }}><RichText.Content value={testimonial.company} /></p>
		  )}
          </div>
        </div>
    );
